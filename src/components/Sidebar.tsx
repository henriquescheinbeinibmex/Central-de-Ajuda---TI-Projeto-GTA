  ...
  const navItems = [
    {
      ...
      roles: ["COLABORADOR", "CONSULTOR_TI", "GESTOR"],
    },
    {
    ...
    },
    {
      ...
      roles: ["CONSULTOR_TI", "GESTOR"],
    },
    {
      href: "/admin/categorias",
      label: "Categorias e Setores",
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="p-5 border-b border-slate-700">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="font-semibold text-sm leading-tight">Central de Ajuda</p>
            <p className="text-slate-400 text-xs">de TI</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-slate-700">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-8 h-8 bg-slate-600 rounded-full flex items-center justify-center flex-shrink-0">
            <span className="text-sm font-medium">{nome.charAt(0).toUpperCase()}</span>
          </div>
          <div className="min-w-0">
            <p className="text-sm font-medium truncate">{nome}</p>
            <p className="text-xs text-slate-400">
              {role === "CONSULTOR_TI" ? "Consultor de TI" : role === "GESTOR" ? "Gestor" : "Colaborador"}
            </p>
          </div>
        </div>
        <button
